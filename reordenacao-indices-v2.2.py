...
"""
================================================================================
PIPELINE UNIFICADO DE REORDENAÇÃO DE ESTRUTURA CAR-T - MULTI-FRAME (v2.2)
================================================================================
...
DATA: 2025
VERSÃO: 2.2 - Correções e otimizações no processamento XTC

NOVIDADES v2.2:
- Correção do mapeamento de chain: GRO segid -> PDB chainID
- Correção do mapeamento de resseq: numeração global -> per-chain
- Otimização: converte apenas átomos no escopo (~136) em vez de todos (~467k)
- Otimização: usa NumPy para extração/escrita de coordenadas em bulk
- Auto-detecção de resíduos alvo por sequência de aminoácidos (ILE-THR-LEU-TYR-CYS-LYS-ARG)
- Cálculo dinâmico de start_serial/end_serial a partir dos resíduos encontrados
- Escrita de frames XTC com flush correto via universe.trajectory.ts
...
# Tentar importar NumPy para otimizações de performance
try:
    import numpy as np
    HAS_NUMPY = True
except ImportError:
...
# ==============================================================================
# SEÇÃO 2C: FUNÇÕES OTIMIZADAS DE PROCESSAMENTO XTC (v2.2)
# ==============================================================================

# Sequência esperada de resíduos alvo para auto-detecção
EXPECTED_RESIDUE_SEQUENCE = ['ILE', 'THR', 'LEU', 'TYR', 'CYS', 'LYS', 'ARG']


def detect_topology_type(topology_file: str) -> str:
    """
    Detecta o tipo de topologia pelo extensão do arquivo.

    Returns:
        'pdb' ou 'gro'
    """
    ext = os.path.splitext(topology_file)[1].lower()
    if ext == '.pdb':
        return 'pdb'
    elif ext == '.gro':
        return 'gro'
...
def find_target_residues_in_universe(universe: 'mda.Universe',
                                     expected_sequence: List[str] = None) -> Optional[int]:
    """
    Localiza a sequência de resíduos alvo no Universe por correspondência de nomes.

    Busca padrão sequencial de aminoácidos (ILE-THR-LEU-TYR-CYS-LYS-ARG)
    independente de numeração ou chain ID.

    Args:
        universe: Objeto Universe do MDAnalysis
        expected_sequence: Lista de nomes de resíduos esperados

    Returns:
        Índice do primeiro resíduo da sequência no array universe.residues,
        ou None se não encontrado
    """
    if expected_sequence is None:
        expected_sequence = EXPECTED_RESIDUE_SEQUENCE

    residues = universe.residues
    n_expected = len(expected_sequence)

    for i in range(len(residues) - n_expected + 1):
        match = True
        for j, expected_name in enumerate(expected_sequence):
            if residues[i + j].resname != expected_name:
                match = False
                break
        if match:
            return i
...
def prepare_xtc_atom_mapping(universe: 'mda.Universe', topology_type: str,
                              target_chain: str = 'B',
                              res_start: int = 576, res_end: int = 583,
                              verbose: bool = True) -> Dict:
    """
    Pré-computa o mapeamento de átomos para processamento XTC otimizado.

    Resolve os problemas de chain mapping e resseq mapping:
    - Para topologia PDB: usa chainID e resSeq diretamente
    - Para topologia GRO: auto-detecta resíduos pela sequência de aminoácidos
      e mapeia para a numeração PDB esperada

    Args:
        universe: Objeto Universe do MDAnalysis
        topology_type: 'pdb' ou 'gro'
        target_chain: Chain alvo (ex: "B")
        res_start: Primeiro resíduo do escopo (ex: 576)
        res_end: Último resíduo do escopo (ex: 583)
        verbose: Mostrar informações de mapeamento

    Returns:
        Dict com:
        - 'scope_mda_indices': np.array com índices MDAnalysis dos átomos no escopo
        - 'atom_mapping': dict {mda_index -> {serial, chain, resseq, name, resname}}
        - 'start_serial': serial do primeiro N do backbone
        - 'end_serial': serial do último C do backbone
        - 'n_scope_atoms': número de átomos no escopo
    """
    num_residues = res_end - res_start + 1
    atom_mapping = {}
    scope_mda_indices = []

    if topology_type == 'pdb':
        # PDB: chainID e resSeq são preservados corretamente
        if verbose:
            print(f"  Topologia PDB: usando chainID e resSeq direto")

        for atom in universe.atoms:
            chain_id = atom.chainID if hasattr(atom, 'chainID') else ''
            resseq = atom.resid

            if chain_id == target_chain and res_start <= resseq <= res_end:
                mda_idx = atom.index
                scope_mda_indices.append(mda_idx)
                atom_mapping[mda_idx] = {
                    'serial': atom.id if hasattr(atom, 'id') else atom.index + 1,
                    'chain': chain_id,
                    'resseq': resseq,
                    'name': atom.name,
        ...
        if not scope_mda_indices:
            raise RuntimeError(
                f"Nenhum átomo encontrado para chain={target_chain}, "
                f"resíduos {res_start}-{res_end} na topologia PDB."
            )

        # Encontrar start_serial e end_serial
        start_serial = None
        end_serial = None
        for mda_idx, info in atom_mapping.items():
            if info['resseq'] == res_start and info['name'] == 'N':
                start_serial = info['serial']
            if info['resseq'] == res_end and info['name'] == 'C':
                end_serial = info['serial']

    elif topology_type == 'gro':
        # GRO: não tem chainID, resid é global
        # Auto-detectar pela sequência de aminoácidos
        if verbose:
            print(f"  Topologia GRO: auto-detectando resíduos por sequência de aminoácidos...")

        found_start_idx = find_target_residues_in_universe(universe)

        if found_start_idx is None:
            raise RuntimeError(
                f"Não foi possível encontrar a sequência de resíduos alvo "
                f"({', '.join(EXPECTED_RESIDUE_SEQUENCE)}) na topologia GRO.\n"
                f"Verifique se os nomes dos resíduos na topologia estão corretos."
            )

        residues = universe.residues

        if verbose:
            print(f"  Sequência encontrada a partir do resíduo "
                  f"#{found_start_idx} ({residues[found_start_idx].resname} "
                  f"resid={residues[found_start_idx].resid})")

        # Mapear resíduos GRO para numeração PDB
        # Usar serial sequencial baseado na posição no escopo
        serial_counter = 1
        serial_map_by_name = {}  # (pdb_resseq, atom_name) -> serial

        for offset in range(num_residues):
            if found_start_idx + offset >= len(residues):
                raise RuntimeError(
                    f"Topologia GRO tem apenas {len(residues)} resíduos, "
                    f"mas precisamos do resíduo no índice {found_start_idx + offset}."
                )

            gro_residue = residues[found_start_idx + offset]
            pdb_resseq = res_start + offset

            if verbose and offset < len(EXPECTED_RESIDUE_SEQUENCE):
                print(f"    GRO resid={gro_residue.resid} ({gro_residue.resname}) "
                      f"-> PDB resseq={pdb_resseq} chain={target_chain}")

            for atom in gro_residue.atoms:
                mda_idx = atom.index
                ...
                atom_mapping[mda_idx] = {
                    ...
                    'resseq': pdb_resseq,
                    'name': atom.name,
                    'resname': gro_residue.resname,
                }
                serial_map_by_name[(pdb_resseq, atom.name)] = serial

        # Encontrar start_serial e end_serial
        start_serial = serial_map_by_name.get((res_start, 'N'))
        end_serial = serial_map_by_name.get((res_end, 'C'))

        if start_serial is None:
            raise RuntimeError(f"Átomo N do resíduo {res_start} não encontrado na topologia GRO.")
        if end_serial is None:
            raise RuntimeError(f"Átomo C do resíduo {res_end} não encontrado na topologia GRO.")

    else:
        raise ValueError(
            f"Tipo de topologia '{topology_type}' não suportado. Use PDB ou GRO."
        )

    scope_mda_indices = sorted(scope_mda_indices)

    if verbose:
        print(f"  Átomos no escopo: {len(scope_mda_indices)}")
        print(f"  start_serial={start_serial}, end_serial={end_serial}")

    result = {
        'scope_mda_indices': scope_mda_indices,
        'atom_mapping': atom_mapping,
        'start_serial': start_serial,
        'end_serial': end_serial,
        'n_scope_atoms': len(scope_mda_indices),
    }

    if HAS_NUMPY:
        result['scope_mda_indices_np'] = np.array(scope_mda_indices, dtype=int)

    return result


def xtc_frame_to_atoms_scoped(universe: 'mda.Universe',
                               mapping: Dict) -> Tuple[List[Dict], List[str]]:
    """
    Converte APENAS os átomos no escopo do frame atual para formato dict.

    Otimização: em vez de converter todos os ~467k átomos, converte apenas
    os ~136 átomos nos resíduos alvo (576-583).

    Usa NumPy para extração em bulk das coordenadas quando disponível.

    Args:
        universe: Universe com frame já selecionado
        mapping: Resultado de prepare_xtc_atom_mapping()

    Returns:
        Tupla (lista de átomos formato dict, linhas PDB simuladas)
    """
    atom_mapping = mapping['atom_mapping']
    scope_indices = mapping['scope_mda_indices']

    atoms = []
    lines = []

    if HAS_NUMPY and 'scope_mda_indices_np' in mapping:
        # Extração NumPy em bulk: pega todas as coordenadas de uma vez
        all_positions = universe.atoms.positions  # array (N, 3)
        scope_positions = all_positions[mapping['scope_mda_indices_np']]

        for local_idx, mda_idx in enumerate(scope_indices):
            info = atom_mapping[mda_idx]
            x, y, z = scope_positions[local_idx]
        ...
        # Fallback sem NumPy: acesso átomo a átomo
        for local_idx, mda_idx in enumerate(scope_indices):
            info = atom_mapping[mda_idx]
            atom = universe.atoms[mda_idx]
            x, y, z = atom.position
...
def write_back_to_universe(universe: 'mda.Universe', atoms: List[Dict],
                            mapping: Dict) -> None:
    """
    Escreve as coordenadas processadas de volta no Universe usando NumPy.

    Otimização: em vez de um loop O(N*M) comparando seriais, usa mapeamento
    direto serial -> índice MDAnalysis para escrita em O(N).

    Args:
        universe: Universe do MDAnalysis
        atoms: Lista de átomos processados (formato dict)
        mapping: Resultado de prepare_xtc_atom_mapping()
    """
    scope_indices = mapping['scope_mda_indices']

    # Construir mapeamento serial -> coordenadas processadas
    serial_to_coords = {}
    for atom in atoms:
        serial_to_coords[atom['serial']] = (atom['x'], atom['y'], atom['z'])

    # Construir mapeamento serial -> índice MDAnalysis (inverso do atom_mapping)
    serial_to_mda_idx = {}
    for mda_idx, info in mapping['atom_mapping'].items():
        serial_to_mda_idx[info['serial']] = mda_idx

    if HAS_NUMPY:
        # Escrita NumPy em bulk
        positions = universe.atoms.positions  # referência ao array de posições

        for serial, (x, y, z) in serial_to_coords.items():
            mda_idx = serial_to_mda_idx.get(serial)
            if mda_idx is not None:
                positions[mda_idx] = [x, y, z]

        # Atualizar posições no Universe
        ...
        for serial, (x, y, z) in serial_to_coords.items():
            mda_idx = serial_to_mda_idx.get(serial)
            if mda_idx is not None:
                universe.atoms[mda_idx].position = [x, y, z]
...
def executar_pipeline_xtc(topology_file: str, xtc_entrada: str, xtc_saida: str,
                          ...
                          chain: str = "B", res_start: int = 576, res_end: int = 583,
                          verbose: bool = True) -> None:
    """
    Executa o pipeline completo para trajetórias em formato XTC (v2.2 otimizado).

    CORREÇÕES v2.2:
    1. Chain mapping: auto-detecta chainID para topologias GRO
    2. Resseq mapping: auto-detecta numeração de resíduos pela sequência de aminoácidos
    3. Performance: converte apenas átomos no escopo (~136) em vez de todos (~467k)
    4. Performance: usa NumPy para extração/escrita de coordenadas em bulk
    5. Frame writing: escrita correta com timestep do Universe
    ...
    Args:
        ...
        start_serial: Serial do primeiro átomo do backbone (usado apenas se não auto-detectado)
        end_serial: Serial do último átomo do backbone (usado apenas se não auto-detectado)
        chain: Cadeia alvo
        res_start: Primeiro resíduo do escopo (ex: 576)
        res_end: Último resíduo do escopo (ex: 583)
    ...
    print("="*80)
    print("PIPELINE UNIFICADO DE REORDENAÇÃO CAR-T - XTC (v2.2 otimizado)")
    print("="*80)
    ...
    # --- FIX 1 & 2: Detectar tipo de topologia e construir mapeamento ---
    topology_type = detect_topology_type(topology_file)
    if topology_type == 'unknown':
        print(f"AVISO: Tipo de topologia não reconhecido para '{topology_file}'.")
        print(f"       Tentando como PDB...")
        topology_type = 'pdb'

    print(f"\nTipo de topologia detectado: {topology_type.upper()}")
    ...
    total_atoms = universe.atoms.n_atoms
    print(f"Total de átomos no sistema: {total_atoms}")
    ...
    # --- Pré-computar mapeamento de átomos (executado UMA vez) ---
    print(f"\nPré-computando mapeamento de átomos...")
    try:
        mapping = prepare_xtc_atom_mapping(
            universe, topology_type,
            target_chain=chain,
            res_start=res_start,
            res_end=res_end,
            verbose=verbose
        ...
        print(f"ERRO no mapeamento de átomos: {e}")
        sys.exit(1)

    # Usar start_serial/end_serial do mapeamento (auto-detectados)
    mapped_start_serial = mapping['start_serial']
    mapped_end_serial = mapping['end_serial']
    n_scope = mapping['n_scope_atoms']

    print(f"\nMapeamento concluído:")
    print(f"  Átomos no escopo: {n_scope} (de {total_atoms} total)")
    print(f"  start_serial: {mapped_start_serial} (original: {start_serial})")
    print(f"  end_serial: {mapped_end_serial} (original: {end_serial})")
    if HAS_NUMPY:
        print(f"  NumPy: ativado (extração em bulk)")
    else:
        print(f"  NumPy: não disponível (modo fallback)")
    print("="*80)
    ...
    # --- FIX 4: Escrita correta de frames XTC ---
    with XTCWriter(xtc_saida, n_atoms=total_atoms) as writer:
        for frame_idx, ts in enumerate(universe.trajectory):
            ...
            try:
                # --- FIX 3: Converter apenas átomos no escopo ---
                atoms, lines = xtc_frame_to_atoms_scoped(universe, mapping)
                ...
                try:
                    # FASE 1: Backbone (usa serials mapeados)
                    coords, backbone_ids, expansoes = fase1_backbone(
                        atoms, coords, mapped_start_serial, mapped_end_serial,
                        verbose=False
                    )
                    aplicar_coords_para_atoms(atoms, coords)
                    ...
                    for res in residues:
                        for aname in ('HN', 'HA', 'O'):
                            for a in atoms:
                                if a['chain'] == res['chain'] and a['resseq'] == res['resseq'] and a['name'] == aname:
                                    locked_serials.add(a['serial'])
                    coords, locked_serials = fase3_ca_cb(
                        atoms, coords, backbone_ids, locked_serials, verbose=False
                    )
                    aplicar_coords_para_atoms(atoms, coords)

                    # FASES 4-10: Cadeias laterais
                    locked = inicializar_locked_base(atoms)
                    scope_indices = get_scope_indices(atoms, res_start, res_end, chain)

                    processar_ile(atoms, locked, resseq=res_start, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "ILE", res_start, chain)

                    processar_thr(atoms, locked, resseq=res_start + 1, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "THR", res_start + 1, chain)

                    processar_leu(atoms, locked, resseq=res_start + 2, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "LEU", res_start + 2, chain)

                    processar_tyr(atoms, locked, resseq=res_start + 3, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "TYR", res_start + 3, chain)

                    processar_cys(atoms, locked, resseq=res_start + 4, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "CYS", res_start + 4, chain)

                    processar_lys(atoms, locked, resseq=res_start + 5, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "LYS", res_start + 5, chain)

                    processar_arg(atoms, locked, resseq=res_start + 6, chain=chain,
                                  verbose=False, scope_indices=scope_indices)
                    bloquear_residuo(atoms, locked, "ARG", res_start + 6, chain)

                    # --- FIX 3 & 4: Escrever coordenadas de volta via mapeamento ---
                    write_back_to_universe(universe, atoms, mapping)
                ...
                # --- FIX 4: Escrita correta do frame usando timestep ---
    ...
    if frames_com_falha:
        ...
        with open(diag_file, 'w') as f_diag:
            ...
            f_diag.write(f"Topologia: {topology_file} ({topology_type.upper()})\n")
            f_diag.write(f"Total de frames: {total_frames}\n")
            f_diag.write(f"Frames com falha: {len(frames_com_falha)}\n")
            f_diag.write(f"Mapeamento: start_serial={mapped_start_serial}, "
                         f"end_serial={mapped_end_serial}\n")
            f_diag.write("="*80 + "\n\n")
    ...
    if todas_expansoes:
        ...
        with open(expansao_file, 'w') as f_exp:
            ...
            f_exp.write(f"Topologia: {topology_file} ({topology_type.upper()})\n")
            f_exp.write(f"Total de frames: {total_frames}\n")
            f_exp.write(f"Frames com expansão: {frames_com_expansao}\n")
            f_exp.write(f"Total de átomos com expansão: {total_atomos_com_expansao}\n")
            f_exp.write("=" * 90 + "\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description='Pipeline de Reordenação de Índices para Estruturas CAR-T (v2.2)',
...
Exemplos de uso:
  ...
  # Arquivos XTC com topologia PDB (recomendado):
  %(prog)s -t topologia.pdb trajetoria.xtc
  %(prog)s -t topologia.pdb trajetoria.xtc saida.xtc

  # Arquivos XTC com topologia GRO (auto-detecção de chain/resseq):
...
Novidades da versão 2.2:
  - Correção do mapeamento de chain para topologia GRO (segid -> chainID)
  - Correção do mapeamento de resseq para topologia GRO (global -> per-chain)
  - Auto-detecção de resíduos alvo pela sequência de aminoácidos
  - Otimização: converte apenas átomos no escopo (~136 de ~467k)
  - Otimização: usa NumPy para coordenadas em bulk
  - Escrita correta de frames XTC
...
Correções da versão 2.0:
  ...
  - Algoritmo DFS otimizado para reconstrução do backbone
    ...
    parser.add_argument(
        '-t', '--topology',
        dest='topology_file',
        help='Arquivo de topologia (PDB recomendado, GRO suportado) - obrigatório para arquivos XTC'
    )
    parser.add_argument(
        '-q', '--quiet',
        action='store_true',
        help='Modo silencioso (reduz output)'
    )
    parser.add_argument(
        '-v', '--version',
        action='version',
